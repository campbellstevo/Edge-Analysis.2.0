...
def _monte_carlo(df, styler) -> None:
    ...
    risk = t._slider_row(
        "Risk per trade",
        lambda: st.number_input("Risk per trade", min_value=0.25, max_value=5.0,
                                value=1.0, step=0.25, format="%.2f",
                                key="pro_mc_risk", label_visibility="collapsed"),
        unit="% of balance, per trade")
    n_tr = t._slider_row(
        "Trades to project",
        lambda: st.number_input("Trades to project", min_value=50, max_value=1000,
                                value=300, step=25, key="pro_mc_n",
                                label_visibility="collapsed"),
        unit="trades")
    start = t._slider_row(
        "Starting balance",
        lambda: st.number_input("Starting balance", min_value=1_000, max_value=200_000,
                                value=10_000, step=500, key="pro_mc_bal",
                                label_visibility="collapsed"),
